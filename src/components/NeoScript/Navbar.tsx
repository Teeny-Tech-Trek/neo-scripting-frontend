        ...
        <div className="flex items-center gap-1.5" id="tour-user-menu">
          ...
          {isAuthenticated && (
            <a
              ...
              className={[
                "flex items-center gap-2 px-4 py-2 rounded-full text-[14px] font-semibold transition-all duration-200",
                "border-2 bg-violet-500/[0.08] hover:bg-violet-500/[0.14]",
                credits.balance !== null && !credits.unlimited && credits.balance < 5
                  ? "border-amber-400/60 text-amber-200 hover:border-amber-400/80"
                  : "border-violet-500/50 text-violet-100 hover:border-violet-400/80",
              ].join(" ")}
              aria-label="View billing and credit balance"
            >
              {credits.unlimited ? (
                <InfinityIcon size={15} strokeWidth={2.5} />
              ) : (
