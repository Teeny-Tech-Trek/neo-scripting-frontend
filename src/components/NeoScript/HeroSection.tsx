...
function BriefForm({ onGenerate }: { onGenerate: (d: BriefData) => void }) {
  ...
  // Competitor chips. Each entry is a competitor name or URL — the Researcher
  // agent crawls/compares against these to ground the post in the market.
  const COMPETITOR_MAX = 5;
  const COMPETITOR_LEN_MAX = 120;
  ...
  const addCompetitor = (raw: string) => {
    const cleaned = raw.trim().replace(/\s+/g, " ");
    if (!cleaned) return;
    if (cleaned.length > COMPETITOR_LEN_MAX) return;
    const dup = competitors.some((c) => c.toLowerCase() === cleaned.toLowerCase());
    if (dup) return;
    if (competitors.length >= COMPETITOR_MAX) return;
  ...
  const handleCompetitorKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" || e.key === ",") {
      ...
      setCompetitorDraft("");
    } else if (e.key === "Backspace" && !competitorDraft && competitors.length > 0) {
      e.preventDefault();
  ...
  const handleSubmit = () => {
    ...
    // Same logic for competitors — commit any unsubmitted draft chip.
    let finalCompetitors = competitors;
    const pendingComp = competitorDraft.trim();
    if (pendingComp && competitors.length < COMPETITOR_MAX) {
      const dup = competitors.some((c) => c.toLowerCase() === pendingComp.toLowerCase());
      if (!dup && pendingComp.length <= COMPETITOR_LEN_MAX) {
        finalCompetitors = [...competitors, pendingComp];
      }
    }
    setSubmitting(true);
    onGenerate({
        ...
        <div className="pb-7 border-b border-white/[0.06]">
          ...
          >
            ...
            <span className="text-[12px] text-white/35">
              keywords · competitors · reference doc
            </span>
          </button>
          ...
          {advancedOpen && (
            <div className="mt-4 space-y-5">
              ...
              <div>
                ...
                <p className="text-[12px] text-white/45 mb-2 leading-relaxed">
                  Press Enter or comma to add. Paste a competitor name or URL
                  — the Researcher agent compares against each one to ground
                  the post in your market. Max {COMPETITOR_MAX} entries.
                </p>
                ...
                >
                  ...
                  {competitors.length < COMPETITOR_MAX ? (
                    <input
                      ...
                      onChange={(e) =>
                        setCompetitorDraft(e.target.value.slice(0, COMPETITOR_LEN_MAX))
                      }
                      onKeyDown={handleCompetitorKeyDown}
                      onPaste={handleCompetitorPaste}
                      onBlur={() => {
                        if (competitorDraft.trim()) {
                          addCompetitor(competitorDraft);
                          setCompetitorDraft("");
                        }
                      }}
                      placeholder={
                        competitors.length === 0
                          ? 'e.g. "stripe.com", "Zapier", "openai.com"'
                          : "Add another…"
                      }
                      className="flex-1 min-w-[140px] bg-transparent text-[13px] text-white placeholder:text-white/30 outline-none py-0.5"
                    />
                  ) : (
                    <span className="text-[12px] text-white/40 italic">
                      Maximum reached — remove one to add more.
